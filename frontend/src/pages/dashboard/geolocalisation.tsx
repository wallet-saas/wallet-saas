...
export default function GeolocalisationPage() {
  ...
  // Settings state
  const [rayon, setRayon] = useState(commercant?.rayon_geoloc_metres ?? 200);
  const [latitude, setLatitude] = useState<string>(commercant?.latitude?.toString() ?? '');
  const [longitude, setLongitude] = useState<string>(commercant?.longitude?.toString() ?? '');
  const [message, setMessage] = useState(commercant?.geoloc_message ?? '');
  const [heureDebut, setHeureDebut] = useState<number>(commercant?.geoloc_heure_debut ?? 8);
  const [heureFin, setHeureFin] = useState<number>(commercant?.geoloc_heure_fin ?? 22);
  ...
  // Sync settings state when commercant changes
  useEffect(() => {
    if (commercant) {
  ...
  const handleToggleModule = useCallback(async (checked: boolean) => {
    setToggling(true);
    try {
        ...
        <div className="space-y-6">
                    ...
                    {[
                      {
                        step: '1',
                        title: 'Configuration',
                        desc: 'Activez le module dans les Paramètres et définissez votre adresse GPS et le rayon de détection (50m – 1000m).',
              ...
              <div className="flex items-start gap-3 bg-blue-50 rounded-xl px-5 py-4 border border-blue-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-blue-600 mt-1">
                    Configurez la latitude/longitude de votre commerce et le rayon de détection dans l'onglet Paramètres.
          ...
          {activeTab === 'parametres' && (
            <div className="space-y-6">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Rayon de détection</CardTitle>
                </CardHeader>
                <CardBody>
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-500">
                      Définissez la distance à laquelle la notification sera envoyée aux clients à proximité.
                      <strong> 100-200m</strong> est recommandé en ville, <strong>300-500m</strong> en zone péri-urbaine.
                    </p>
                  </div>
                </CardBody>
              </Card>
                ...
                <CardBody>
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-gray-400">
                      Indiquez les coordonnées GPS exactes de votre commerce pour des notifications précises.
                    </p>
                  </div>
                </CardBody>
              </Card>

              {/* Notification message & schedule */}
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                ...
                <CardBody>
                  <div className="space-y-5">
                    <Input
                      ...
                      placeholder="Ex: 🎉 Bonjour ! Passez nous voir, vous pouvez gagner des points !"
                      value={message}
                      onChange={(e) => setMessage(e.target.value)}
                      hint="Ce message apparaîtra dans la notification push envoyée aux clients à proximité."
                    />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        />
                        <p className="text-xs text-gray-400 mt-1 text-center">0 – 23</p>
                      </div>
                      <div>
                        <label className="label flex items-center gap-1.5">
                          <Clock className="h-3.5 w-3.5 text-gray-400" />
                    ...
                    <p className="text-xs text-gray-500">
                      Les notifications ne seront envoyées qu'entre ces deux heures (format 24h).
                    </p>
                  </div>
                </CardBody>
              </Card>
              ...
              <div className="flex items-start gap-3 bg-blue-50 rounded-xl px-5 py-4 border border-blue-100">
                ...
                <div className="space-y-1">
                  <p className="text-sm font-medium text-blue-700">Comment fonctionne la géolocalisation ?</p>
                  <ul className="text-xs text-blue-600 space-y-1 list-disc list-inside">
                    <li>Le module détecte les clients équipés de votre carte de fidélité qui se trouvent dans votre rayon de détection.</li>
                    <li>Une notification push personnalisée est envoyée automatiquement sur leur téléphone.</li>
                    <li>Le message, le rayon et les horaires sont entièrement configurables ci-dessus.</li>
                    <li>Les statistiques de l'onglet &quot;Statistiques&quot; vous permettent de suivre les performances en temps réel.</li>
                  </ul>
                </div>
              </div>
