...
export function SocialProof() {
  ...
  return (
    <section className={`py-16 sm:py-20 relative overflow-hidden border-y ${t.border}`}>
      ...
      <div className="relative flex overflow-hidden h-28 sm:h-32 items-center">
        <motion.div
          animate={{ x: ["0%", "-33.33%"] }}
          transition={{ ease: "linear", duration: 25, repeat: Infinity }}
          className="flex whitespace-nowrap gap-4 sm:gap-8 pl-4 sm:pl-8"
        >
          {duplicatedTypes.map((type, i) => (
            <motion.div
              key={i}
              whileHover={{ scale: 1.05 }}
              className={`group flex shrink-0 items-center gap-3 sm:gap-4 px-4 sm:px-8 py-3 sm:py-4 rounded-2xl ${t.cardBg} ${t.cardBorder} backdrop-blur-md shadow-xl transition-all duration-300 cursor-pointer`}
            >
