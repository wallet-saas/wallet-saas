...
import { PremiumCardPreview, CardFormat } from './PremiumCardPreview';
            ...
            <div>
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-xl overflow-hidden cursor-pointer transition-all hover:border-indigo-400 ${
                  design.background_image_url ? 'border-indigo-300' : 'border-gray-300'
              ...
              <div className="flex items-center gap-4">
                <div
                  ...
                  className={`relative w-20 h-20 border-2 border-dashed rounded-xl overflow-hidden cursor-pointer transition-all hover:border-indigo-400 flex-shrink-0 ${
                    design.logo_url ? 'border-indigo-300' : 'border-gray-300'
                  } ${readOnly ? 'cursor-default' : ''}`}
                >
                  {design.logo_url ? (
        ...
        {hasGWLimitations && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-amber-700">Limites Google Wallet</p>
      ...
      <div className="lg:col-span-3 lg:sticky lg:top-4 lg:self-start space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5">
            <button onClick={() => setPreviewFormat('google')}
              className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${previewFormat === 'google' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              Google Wallet
            </button>
            <button onClick={() => setPreviewFormat('apple')}
              className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${previewFormat === 'apple' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
              Apple Wallet
            </button>
          </div>
        </div>
