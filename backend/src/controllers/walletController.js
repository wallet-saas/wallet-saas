...
async function generateWalletUrls(carte, commercant) {
  const [googleUrl, appleUrl] = await Promise.all([
    googleWalletService.generateSaveUrl(carte, commercant).catch(() => null),
...
const getInstallPage = async (req, res) => {
  try {
    ...
    const { data: card, error: cardError } = await supabase
      .from('cartes')
      .select('id, pass_serial_number, points, google_wallet_url, apple_wallet_url, commercants(id, nom_enseigne, carte_couleur_primaire)')
    ...
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3001';
    const commercantId = card.commercants?.id;
    ...
    const couleurPrimaire = card.commercants?.carte_couleur_primaire || '#6366f1';
    const nomEnseigne = card.commercants?.nom_enseigne || 'Votre commerce';
...
const downloadPass = async (req, res) => {
  try {
    ...
    const { data: card, error } = await supabase
      .from('cartes')
      .select('pass_serial_number, points, google_wallet_url, apple_wallet_url, commercants(nom_enseigne, carte_couleur_primaire, carte_couleur_secondaire, points_recompense)')
      .eq('pass_serial_number', serial)
      .single();
    ...
    res.status(200).json({
      ...
      data: {
        ...
        points: card.points,
        nom_enseigne: card.commercants.nom_enseigne,
...
const generateCardForClient = async (req, res) => {
  try {
    ...
    const { data: commercant, error: commercantError } = await supabase
      .from('commercants')
      .select('id, nom_enseigne, carte_couleur_primaire, carte_couleur_secondaire, carte_logo_url, points_recompense, abonnement_statut')
      .eq('id', commercantId)
      .single();
