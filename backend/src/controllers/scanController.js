...
const scanQR = async (req, res) => {
  try {
    ...
    const { data: carte, error: carteError } = await supabase
      .from('cartes')
      .select('id, pass_serial_number, points, commercant_id, actif, commercants(points_recompense)')
    ...
    // Seuil de récompense du commerçant (défaut 10)
    const seuil = carte.commercants?.points_recompense || 10;
